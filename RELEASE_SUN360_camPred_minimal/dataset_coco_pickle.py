...
pickle_paths = {\
    # 'train-val': '/data/COCO/coco_results/results_with_kps_20200208_morethan2_2-8/pickle', # 13875 \
    # 'train-val': '/data/COCO/coco_results/results_with_kps_20200402_filtered_ratio2-8_moreThan2/pickle', # 11347 \
    'train-val': '/data/COCO/coco_results/results_with_kps_20200403_noNull_filtered_ratio2-8_moreThan2/pickle', # 10914 \
...
# pickle_paths['test'] = '/newfoundland/results_extra/results_coco/results_test_20200302_Car_noSmall-ratio1-35-mergeWith-results_with_kps_20200225_train2017_detOnly_filtered_2-8_moreThan2/pickle' #MultiCat
...
class COCO2017Scale(torchvision.datasets.coco.CocoDetection):
    def __init__(self, transforms_maskrcnn=None, transforms_yannick=None, split='', shuffle=True, logger=None, opt=None):
        ...
        if self.train:
            ...
            assert len(self.pickle_files) == len(self.pickle_files) == len(self.img_files) == len(self.yannick_mat_files)

            # bbox_npy_files_filtered = []
            pickle_files_filtered = []
            img_files_filtered = []
            yannick_mat_files_filtered = []
            for pickle_file, img_file, yannick_mat_file in zip(self.pickle_files, self.img_files, self.yannick_mat_files):
                if os.path.isfile(pickle_file):
                    assert os.path.basename(pickle_file)[:12] == os.path.basename(img_file)[:12] == os.path.basename(yannick_mat_file)[:12]
        ...
        assert len(self.pickle_files) == len(self.img_files) == len(self.yannick_mat_files)
        ...
        if shuffle:
            ...
            list_zip = list(zip(self.img_files, self.pickle_files, self.yannick_mat_files))
            random.shuffle(list_zip)
            self.img_files, self.pickle_files, self.yannick_mat_files = zip(*list_zip)
            assert os.path.basename(self.img_files[0])[:12] == os.path.basename(self.pickle_files[0])[:12] == os.path.basename(self.yannick_mat_files[0])[:12]
        # if train:
